...
function CheckOverflow(slot0)
	...
	if (slot0[DROP_TYPE_RESOURCE][PlayerConst.ResOil] or 0) > 0 and slot5:OilMax(slot1) then
		return false, i18n("oil_max_tip_title") .. i18n("resource_max_tip_mail")
	end

	if slot2 > 0 and slot5:GoldMax(slot2) then
...
function CheckShipExpOverflow(slot0)
	...
	for slot5, slot6 in pairs(slot0[DROP_TYPE_ITEM]) do
		if pg.item_data_statistics[slot5].max_num < slot1:getItemCountById(slot5) + slot6 then
			return false
		end
	end
		...
		[DROP_TYPE_ITEM] = function ()
			if uv0.cfg.type == Item.SKIN_ASSIGNED_TYPE then
				if Item.InTimeLimitSkinAssigned(uv0.id) then
					slot1 = table.mergeArray(slot0[2], pg.item_data_template[uv0.id].usage_arg[3], true)
				end
