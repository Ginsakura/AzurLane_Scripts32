...
function slot0.GetRawPutList(slot0)
	...
	if not slot0.putInfo then
		...
		slot0.putInfo = _.select((nil or require("GameCfg.backyardTheme.theme_empty"))["furnitures_" .. getProxy(DormProxy):getRawData().level] or {}, function (slot0)
...
function slot0.CheckData(slot0)
	...
	for slot7, slot8 in pairs(slot0.furnitruesByIds) do
		if not getProxy(DormProxy):getRawData():IsPurchasedFurniture(slot8.configId) then
