...
function slot0.GetAttributes(slot0)
	...
	return {
		[slot5] = slot0:getConfig("attribute_" .. slot5) ~= nil and {
			type = slot6,
...
function slot0.GetInfoTrans(slot0, slot1)
	...
	if slot0.type == AttributeType.CD then
		slot3 = uv0.CalcWeanponCD(slot0.value, slot1) .. "s" .. pg.equip_data_code.WAVE.text
...
function slot0.GetWeaponPageInfo(slot0, slot1, slot2)
	...
	for slot9, slot10 in ipairs(slot4.exhibition_list) do
		if not slot3 then
			slot0:GetWeaponInfo(slot10, slot2, pg.equip_bullet_type[slot1].exhibition_type == 2).sub = {}
		else
