...
function slot0.InitSwitch()
			...
			return switch(slot0:getConfig("virtual_type"), {
				...
				[13] = function ()
					if not SkinCouponActivity.StaticExistActivity() then
						pg.TipsMgr.GetInstance():ShowTips(i18n("coupon_timeout_tip", uv0:getName()))
			...
			switch(slot0:getConfig("virtual_type"), {
				...
				[14] = function ()
					if WorldBossConst.WORLD_BOSS_ITEM_ID == uv0.id then
						nowWorld():GetBossProxy():AddSummonPt(uv0.count)
		...
		[DROP_TYPE_SKIN_TIMELIMIT] = function (slot0)
			if getProxy(ShipSkinProxy):getSkinById(slot0.id) and slot4:isExpireType() then
				slot3:addSkin(ShipSkin.New({
					id = slot1,
					end_time = slot0.count + slot4.endTime
				}))
	...
	uv0.MsgboxIntroCase = {
		...
		[DROP_TYPE_ITEM] = function (slot0, slot1, slot2)
			if slot0:getConfig("type") == Item.LOVE_LETTER_TYPE then
				slot3 = string.gsub(slot0:getConfig("display"), "$1", ShipGroup.getDefaultShipNameByGroupID(slot0.extra))
		...
		[DROP_TYPE_STRATEGY] = function (slot0, slot1, slot2)
			for slot7, slot8 in ipairs({
				...
				slot3 = string.gsub(slot0:getConfig("desc"), "$" .. slot7, slot8)
			end
