...
function slot1.SetArgs(slot0, slot1, slot2)
	...
	if slot3.initialCD then
		slot0._nextEffectTime = pg.TimeMgr.GetInstance():GetCombatTime()
	else
...
function slot1.FilterSpWeapon(slot0, slot1)
	function ()
		...
		if uv3.type and not table.contains(uv3.type, uv2.Battle.BattleDataFunction.GetSpWeaponDataFromID(uv0:GetConfigID()).type) then
			uv1 = false
		end
