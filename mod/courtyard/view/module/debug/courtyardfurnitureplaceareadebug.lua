...
function slot0.Flush(slot0)
	...
	for slot7, slot8 in ipairs(slot0.furniture:GetCanputonPosition()) do
		...
		setParent(slot9, slot0:GetView():GetRect():Find("grids"))

		tf(slot9).localScale = Vector3.one
		tf(slot9).localPosition = CourtYardCalcUtil.Map2Local(slot8) + (uv0 and slot0.furniture:RawGetOffset() or Vector3.zero)
