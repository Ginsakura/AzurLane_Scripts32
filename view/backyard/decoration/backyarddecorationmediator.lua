...
function slot0.register(slot0)
	...
	slot0:bind(uv0.SAVE_THEME, function (slot0, slot1, slot2)
		...
		slot5, slot6 = nil
		slot7 = pg.UIMgr.GetInstance().uiCamera:GetComponent(typeof(Camera))
...
function slot0.GenUniqueID(slot0, slot1, slot2)
	for slot8 = 0, slot1:GetOwnFurnitureCount(slot2) - 1 do
		if not slot0:GetAllFloorFurnitures(slot1)[BackyardThemeFurniture.GetUniqueId(slot2, slot8)] then
			return slot9
		end
	end
...
function slot0.handleNotification(slot0, slot1)
	...
	if slot1:getName() == CourtYardEvent._SYN_FURNITURE then
		...
		for slot11, slot12 in ipairs(slot3[1]) do
			if slot0.dorm:GetTheme(getProxy(DormProxy).floor):GetFurniture(slot12.id) then
