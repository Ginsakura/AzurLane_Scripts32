...
function slot0.GetData(slot0)
	slot1 = {}
	slot5 = {
		[slot10.id] = slot10:IsPurchased(slot0.dorm:GetPurchasedFurnitures()) and 1 or 0
	}

	for slot9, slot10 in ipairs(getProxy(DormProxy):GetSystemThemes()) do
		if not slot10:IsOverTime() and slot10:MatchSearchKey(getInputText(slot0.searchInput)) then
...
function slot0.UpdateMainPage(slot0, slot1)
	...
	slot0.largeSpLoader:LoadSpriteAsync("BackYardTheme/theme_" .. slot1.id, function (slot0)
...
function slot0.GetAddList(slot0, slot1)
	...
	for slot8, slot9 in ipairs(slot1:GetFurnitures()) do
		if not slot0.dorm:GetPurchasedFurnitures()[slot9] then
...
function slot0.CalcThemePrice(slot0, slot1)
	for slot8, slot9 in ipairs(slot0:GetAddList(slot1)) do
		slot4 = 0 + slot9:getConfig("dorm_icon_price")
		slot3 = 0 + slot9:getPrice(PlayerConst.ResDormMoney)
	end
...
function slot0.OnDestroy(slot0)
	...
	for slot4, slot5 in pairs(slot0.cards or {}) do
		slot5:Dispose()
	end
