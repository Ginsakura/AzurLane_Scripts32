...
function slot0.Ctor(slot0, slot1, slot2, slot3)
	...
	slot0._playerColliderEvenet:AddPointDownFunc(function (slot0, slot1)
...
function slot0.onPickupCake(slot0, slot1, slot2, slot3, slot4)
	...
	for slot8 = 1, #slot3 do
		...
		if slot9.id == slot2 then
			...
			if slot4 then
				setActive(findTF(slot9.tf, "select"), true)
			end
		else
...
function slot0.step(slot0, slot1)
	...
	if CookGameConst.player_use_ai then
		slot0:setCharAction(slot0.playerChar, slot0:getFillterWanted(slot0.playerChar), slot0.playerCakes)
	end
	...
	for slot9 = #slot0.acCakes, 1, -1 do
		...
		if slot11.x < slot12.x and slot18.x < slot12.x then
			slot0.acCakes[slot9].tf.anchoredPosition.x = slot18.x
