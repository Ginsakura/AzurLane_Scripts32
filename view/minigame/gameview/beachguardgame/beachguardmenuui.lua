...
function slot0.Ctor(slot0, slot1, slot2, slot3)
	...
	GetComponent(findTF(slot0.menuUI, "desc"), typeof(Image)):SetNativeSize()
	...
	for slot12 = 1, slot7 do
		slot13 = slot12
		slot14 = tf(instantiate(findTF(slot0.bookUI, "bg/gridTpl")))
...
function slot0.selectGrid(slot0, slot1)
	if slot0.selectGridIndex ~= slot1 then
		...
		for slot6 = 1, #slot0.grids do
			if slot6 == slot1 then
				setActive(findTF(slot0.grids[slot6], "select"), true)
			else
