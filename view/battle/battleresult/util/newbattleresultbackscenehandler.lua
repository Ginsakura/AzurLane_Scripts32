...
function slot0.CheckActBossSystem(slot0, slot1)
	...
	if #_.map(_.values(getProxy(FleetProxy):getActivityFleets()[slot1.actId][slot1.mainFleetId].ships), function (slot0)
...
function slot5(slot0, slot1)
	...
	if slot2:GetStageBonus(slot1) == 0 and getProxy(SettingsProxy):isTipActBossExchangeTicket() == 1 and getProxy(PlayerProxy):getRawData():getResource(pg.activity_event_worldboss[slot2:getConfig("config_id")].ticket) > 0 then
		return true
	end
...
function slot0.handleNotification(slot0, slot1)
	if slot1:getName() == GAME.BOSSRUSH_SETTLE_DONE then
		slot0:ExitRushBossSystem(slot0.contextData, slot1:getBody())
