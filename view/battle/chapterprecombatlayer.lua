...
function slot0.Register(slot0)
	slot0._formationLogic:AddHeroInfoModify(function (slot0, slot1, slot2)
		...
		if slot14 then
			if slot14 % 100 > 0 then
				slot18 = tostring(slot14 / 100) .. "." .. tostring(slot17)
			end
	...
	slot0._formationLogic:AddEndDrag(function ()
		uv0:emit(ChapterPreCombatMediator.ON_SWITCH_SHIP, uv0.chapter.fleet)
	end)
	slot0._formationLogic:AddCheckRemove(function (slot0, slot1)
		slot0()
	end)
	slot0._formationLogic:AddCheckSwitch(function (slot0, slot1, slot2, slot3, slot4)
	...
	slot0._formationLogic:AddCheckBeginDrag(function (slot0, slot1, slot2)
...
function slot0.updateStageView(slot0, slot1)
	...
	if checkExist(pg.expedition_activity_template[slot1], {
		...
		for slot12 = #slot7, 1, -1 do
			if getProxy(ActivityProxy):getActivityById(slot7[slot12][1]) and not slot13:isEnd() then
