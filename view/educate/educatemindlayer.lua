...
function slot0.updateFinishItem(slot0, slot1, slot2)
	...
	setText(slot0:findTF("desc", slot2), slot0.finishTaskVOs[slot1 + 1]:getConfig("name"))
	onButton(slot0, slot0:findTF("get_btn", slot2), function ()
		if not uv0.isClick then
			uv0.isClick = true

			uv0:doAnim(uv1, function ()
			end)
...
function slot0.doAnim(slot0, slot1, slot2)
	...
	LeanTween.alphaCanvas(GetOrAddComponent(slot1, "CanvasGroup"), 0, 0.198):setFrom(1)
	LeanTween.value(go(slot1), slot4.x, slot4.x + 200, 0.264):setOnUpdate(System.Action_float(function (slot0)
