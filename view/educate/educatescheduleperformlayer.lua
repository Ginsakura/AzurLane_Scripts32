...
function slot0.initData(slot0)
	slot0.planCnt = getProxy(EducateProxy):GetCharData():GetNextWeekPlanCnt()
	slot0.curDay = 1
	slot0.curIndex = 1
...
function slot0.didEnter(slot0)
	...
	pg.PerformMgr.GetInstance():SetParamForUI(slot0.__cname)
	slot0:initDayList()
				...
				table.insert(slot2, function (slot0)
					...
					if uv0.isSkip then
						if not uv3:IsPlan() or uv5 then
							pg.PerformMgr.GetInstance():PlayGroupNoHide(uv3:GetPerformance(), slot0, uv3:IsPlan() and uv4.plan_drops or uv4.spec_event_drops or {})
						else
							slot0()
						end
					else
