...
function slot0.GetSystemUnlockTips()
	...
	for slot5, slot6 in pairs(uv0.system_tip_list) do
		if not (PlayerPrefs.GetInt(getProxy(PlayerProxy):getRawData().id .. uv0.system_save_key .. slot5, 0) == 1) and EducateHelper.IsSystemUnlock(slot5) then
...
function slot0.ClearSystemUnlockTips()
	for slot4, slot5 in pairs(uv0.system_tip_list) do
		if PlayerPrefs.HasKey(getProxy(PlayerProxy):getRawData().id .. uv0.system_save_key .. slot4) then
...
function slot0.GetSiteUnlockTipIds()
	...
	for slot5, slot6 in ipairs(uv0.needTipSiteIds) do
		if not (PlayerPrefs.GetInt(getProxy(PlayerProxy):getRawData().id .. uv0.site_save_key .. slot6, 0) == 1) and EducateHelper.IsSiteUnlock(slot6, true) then
...
function slot0.ClearSiteUnlockTipIds()
	for slot4, slot5 in ipairs(pg.child_site.all) do
		if PlayerPrefs.HasKey(getProxy(PlayerProxy):getRawData().id .. uv0.site_save_key .. slot5) then
			PlayerPrefs.DeleteKey(slot6)
			PlayerPrefs.Save()
		end
	end
...
function slot0.GetPlanUnlockTipIds()
	...
	for slot6, slot7 in ipairs(uv0.needTipPlanIds) do
		if not (PlayerPrefs.GetInt(getProxy(PlayerProxy):getRawData().id .. uv0.plan_save_key .. slot7, 0) == 1) then
			slot9 = pg.child_plan[slot7].pre

			if slot9[2] <= getProxy(EducateProxy):GetPlanProxy():GetHistoryCntById(slot9[1]) then
...
function slot0.ClearPlanUnlockTipIds()
	for slot4, slot5 in ipairs(uv0.needTipPlanIds) do
		if PlayerPrefs.HasKey(getProxy(PlayerProxy):getRawData().id .. uv0.plan_save_key .. slot5) then
			PlayerPrefs.DeleteKey(slot6)
			PlayerPrefs.Save()
		end
	end
