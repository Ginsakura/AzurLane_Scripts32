...
function slot0.Notify(slot0)
	...
	if slot0 == uv0.ENTER_SCENE then
		uv0.CollectEnterStory(slot4, {})
		uv0.CollectEnterGuide(slot4, {})
...
function slot0.GetPrevRaceStories()
	...
	for slot5, slot6 in ipairs(pg.activity_template.all) do
		if pg.activity_template[slot6].type == ActivityConst.ACTIVITY_TYPE_VOTE and pg.TimeMgr.GetInstance():parseTimeFromConfig(slot7.time[3]) < pg.TimeMgr.GetInstance():GetServerTime() then
