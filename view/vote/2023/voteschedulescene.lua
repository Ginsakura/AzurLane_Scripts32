...
function slot0.SetUp(slot0, slot1)
	...
	slot0.lineContainer:SetAsLastSibling()
	seriesAsync({
		function (slot0)
...
function slot0.PlayAnimation(slot0, slot1)
	...
	for slot8, slot9 in pairs(slot0.voteIdList or {}) do
		for slot16, slot17 in ipairs(slot0.animationNodes[slot9]) do
			if slot0:GetRaceState(slot9) == uv0 then
	...
	for slot8 = 2, #slot1 - 3, 2 do
		if (0 + 1) % 2 == 0 then
			table.insert(slot2, {
				{
					id = slot1[slot8 + 1],
					dir = (slot8 == slot4 or slot4 < slot8 + 2) and uv3 or uv0
				},
				{
					id = slot1[slot8],
					dir = uv4
...
function slot0.InitScheduleList(slot0)
	...
	slot2 = {
		[slot7] = slot0:GetRaceState(slot7)
	}
