...
function slot0.PlayStory(slot0)
	...
	if slot0.nday == 7 then
		...
		if (slot0.taskProxy:getTaskById(slot0.taskGroup[slot0.nday][1]) or slot0.taskProxy:getFinishTaskById(slot3)):getTaskStatus() == 2 and (slot0.taskProxy:getTaskById(slot4) or slot0.taskProxy:getFinishTaskById(slot4)):getTaskStatus() == 2 then
			slot2 = slot0.nday - 1 + 1
		end
	end
