...
function slot0.GetTaskCountAble()
	...
	function slot5(slot0)
		...
		if not getProxy(TaskProxy):getTaskById(slot0) and not getProxy(TaskProxy):getFinishTaskById(slot0) then
			return false
		end
...
function slot0.updateUI(slot0)
	...
	for slot9 = 1, #slot0.btnPlayers do
		slot10 = slot3:GetPicturePuzzleIds()[slot9]
	...
	for slot14 = 1, #slot7.data1_list do
		if not false and not table.contains(slot7.data2_list, slot8[slot14]) then
			slot10 = true
...
function slot0.updateDetailPanel(slot0)
	...
	for slot12 = 1, #slot0.medalTfs do
		setActive(findTF(slot0.medalTfs[slot12], "icon"), table.contains(slot5.data2_list, slot5:GetPicturePuzzleIds()[slot12]))
	end
...
function slot0.initTaskPanel(slot0)
	...
	for slot9 = 1, uv1 do
		...
		SetParent(slot11, findTF(slot0.taskPanel, "ad/tag/content"))
		...
		for slot11, slot12 in ipairs(slot0.taskGroups[slot5].taskDic) do
			...
			if not slot12.data.before then
				...
				if slot0:getTaskPos(slot14)[1] < slot13[1] then
					...
					setImageColor(findTF(slot15, "line/back"), slot0:checkTaskBeforeComplete(slot14) and Color.New(0.9921568627450981, 0.9647058823529412, 0.8666666666666667) or Color.New(0.48627450980392156, 0.35294117647058826, 0.2901960784313726))
...
function slot0.getGetAbleTask(slot0)
	...
	for slot7 = 1, #slot0:getActiveTask() do
		if getProxy(TaskProxy):getTaskById(slot3[slot7]) and slot8:getTaskStatus() == 1 then
...
function slot0.taskSelectTag(slot0, slot1, slot2)
	...
	if slot2 then
		...
		LeanTween.value(go(slot0._tf), slot6, slot7, 0.3 + slot3 * 0.1):setOnUpdate(System.Action_float(function (slot0)
			uv0.y = slot0
