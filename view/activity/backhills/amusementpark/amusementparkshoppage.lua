...
function slot0.GetActivityShopTip()
	...
	for slot6, slot7 in ipairs(pg.activity_shop_template.all) do
		if slot1.id == slot2[slot7].activity then
			if (slot2[slot7].num_limit == 0 or (table.indexof(slot1.data1_list, slot7) and slot1.data2_list[slot8] or 0) < slot10.num_limit) and slot10.resource_num <= GetOwnedDropCount({
