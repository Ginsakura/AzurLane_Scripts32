...
function slot4(slot0, slot1, slot2)
	if not slot1 or not slot2 then
		setActive(slot0:Find("error"), false)
...
function slot0.updateDispalyPanel(slot0, slot1, slot2)
	...
	elseif slot2 == "code" then
		...
		for slot10, slot11 in ipairs(parseEquipCode(slot0.code)) do
			if slot10 == 6 then
				slot0.codeData[slot10] = slot3 and pg.spweapon_data_statistics[slot11] and SpWeapon.New({
					id = slot11
				}) or false
		...
		for slot10, slot11 in ipairs(slot0.equipData) do
			if slot10 == 6 then
				uv1(slot1:Find("equipments_quick"):GetChild(slot10 - 1), slot11, slot11 and slot11.shipId and getProxy(BayProxy):getShipById(slot11.shipId) or nil)
			else
				uv0(slot12, slot11, slot11 and slot11.shipId and getProxy(BayProxy):getShipById(slot11.shipId) or nil)
			end
