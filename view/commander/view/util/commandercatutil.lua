...
function slot2(slot0, slot1)
	...
	for slot8, slot9 in pairs(getProxy(FleetProxy):getActivityFleets()[slot3.id]) do
		for slot15, slot16 in pairs(slot9:getCommanders()) do
			slot1[slot16.id].sub = slot9:isSubmarineFleet()
			slot1[slot16.id].fleetId = slot9.id % 10
...
function slot5(slot0, slot1)
	...
	for slot9, slot10 in pairs(slot3:GetBossMission():GetFleets()) do
		for slot16, slot17 in pairs(slot11:getCommanders()) do
			slot1[slot17.id].sub = not (slot0.fleets[slot9] or slot10):IsMainFleet()
...
function slot7(slot0, slot1)
	...
	for slot6, slot7 in pairs(slot2) do
		for slot12, slot13 in pairs(slot7:getCommanders()) do
			slot1[slot13.id].sub = slot6 == #slot2
...
function slot8(slot0, slot1)
	...
	for slot7, slot8 in pairs(_.map({
		...
		for slot14, slot15 in pairs(slot8:getCommanders()) do
			slot1[slot15.id].sub = slot8:isSubmarineFleet()
			slot1[slot15.id].fleetId = slot8.id
...
function slot0.GetCommanderList(slot0)
	if CommanderCatScene.FLEET_TYPE_COMMON == slot0.fleetType then
		uv0(slot0, getProxy(CommanderProxy):getData())
	...
	for slot8, slot9 in ipairs(slot0.ignoredIds or {}) do
		-- Nothing
	end
...
function slot0.GetSkillExpAndCommanderExp(slot0, slot1)
	for slot8, slot9 in pairs(slot1) do
		...
		slot3 = 0 + slot10:getDestoryedExp(slot0.groupId)
		slot2 = 0 + slot10:getDestoryedSkillExp(slot0.groupId)
	end
...
function slot0.CalcCommanderConsume(slot0)
	for slot6, slot7 in ipairs(slot0) do
		slot8 = getProxy(CommanderProxy):RawGetCommanderById(slot7)

		assert(slot8, slot7)

		slot2 = 0 + slot8:getUpgradeConsume()
	end
