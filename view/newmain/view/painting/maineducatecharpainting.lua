...
function slot0.OnFirstTimeTriggerEvent(slot0)
	slot2, slot3, slot4 = ChineseCalendar.GetCurrYearMonthDay(pg.TimeMgr.GetInstance():GetServerTime())

	if ChineseCalendar.IsNewYear(slot2, slot3, slot4) and not function ()
	...
	if getProxy(EventProxy):hasFinishState() and slot1 ~= "event_complete" then
		...
		if #slot2 == 0 then
			if getProxy(TaskProxy):getNotFinishCount() and getProxy(TaskProxy):getNotFinishCount() > 0 and slot1 ~= "mission" then
				table.insert(slot0:FilterExistEvents(uv0.IdleEvents), "mission")
			end
		end
	end
